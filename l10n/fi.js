OC.L10N.register(
    "integration_openstreetmap",
    {
    "OpenStreetMap" : "OpenStreetMap",
    "Map location (by OpenStreetMap)" : "Kartan sijainti (OpenStreetMapin toimesta)",
    "OpenStreetMap locations" : "OpenStreetMap-sijainnit",
    "Bad HTTP method" : "Virheellinen HTTP-metodi",
    "Bad credentials" : "Virheelliset kirjautumistiedot",
    "Connected accounts" : "Yhdistetyt tilit",
    "OpenStreetMap integration" : "OpenStreetMap-integraatio",
    "Integration of OpenStreetMap" : "OpenStreetMapin integraatio",
    "OpenStreetMap options saved" : "OpenStreetMap-asetukset tallennettu",
    "Failed to save OpenStreetMap options" : "OpenStreetMap-asetusten tallentaminen epäonnistui",
    "Maptiler API key" : "Maptilerin API-avain",
    "Enable searching for locations" : "Käytä sijaintien etsimistä",
    "Enable OpenStreetMap link previews" : "Käytä OpenStreetMap-linkkien esikatselua",
    "Enable navigation link" : "Näytä navigointipalkissa",
    "Search on the map" : "Etsi kartalta",
    "Search with Nominatim to get an OpenStreetMap link" : "Etsi Nominatimilla saadaksesi OpenStreetMap-linkin",
    "Click on the map to set the start location" : "Napsauta karttaa lisätäksesi aloitussijainnin",
    "Distance: {distance}" : "Etäisyys: {distance}",
    "Duration: {duration}" : "Kesto: {duration}",
    "Link type" : "Linkkityyppi",
    "Generate direction link" : "Luo suuntaohjelinkki",
    "Link could not be copied to clipboard" : "Linkkiä ei voitu kopioida leikepöydälle",
    "Edit route" : "Muokkaa reittiä",
    "Copy direction link" : "Kopioi suuntaohjelinkki",
    "Unknown distance" : "Tuntematon etäisyys",
    "Unknown duration" : "Tuntematon kesto",
    "Include marker" : "Sisällytä merkki",
    "Generate location link" : "Luo sijaintilinkki",
    "By car" : "Autolla",
    "By bike" : "Pyörällä",
    "By foot" : "Kävellen"
},
"nplurals=2; plural=(n != 1);");
