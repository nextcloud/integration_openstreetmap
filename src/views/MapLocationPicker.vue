...
export default {
	...
	props: {
		pickerTitle: {
			type: String,
			default: t('integration_openstreetmap', 'Map location (by OpenStreetMap)'),
		},
		searchPlaceholder: {
			type: String,
			default: t('integration_openstreetmap', 'Search with Nominatim to get an OpenStreetMap link'),
		},
		submitLabel: {
			type: String,
			default: t('integration_openstreetmap', 'Generate location link'),
		},
		showLinkOptions: {
			type: Boolean,
			default: true,
	...
	data() {
		return {
			provider: getProvider('openstreetmap-point'),
	...
	methods: {
		...
		onSearchSubmit(link) {
			...
			this.$emit('submit', { link: finalLink })
		},
